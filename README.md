...
This assignment is based on the implementation of bayesian matting using the Orchard Bouman Cluster. The objective is to detect an image foreground and background by using a trimap.
The project was first done in Matlab and then converted to Python. 
...
# Plot results between Bayesian and Laplacian
...
- If the output matte has either all 0’s or all 1’s it is valid. If the output matte has any other value, it is invalid.
...
We would also like to acknowledge Yung-Yu Chuang, Brian Curless, David H. Salesin and Richard Szeliski for the work they have done in the paper titled: A Bayesian Approach to Digital Matting.
